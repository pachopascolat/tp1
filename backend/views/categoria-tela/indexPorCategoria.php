...
//            ],
            [
                ...
                'format' => 'raw',
                'value' => function ($model, $key, $index, $column) {
                    return Html::a("({$model->tela->getCantidadDisenios()}) Diseños", ['/galeria/update-galerias','tipo'=> common\models\Galeria::DISENIO, 'tela_id' => $model->tela_id, 'categoria_padre' => $model->categoria->categoria_padre], ['class' => 'btn btn-default']);
                }
            ],
            [
                'label' => 'Modelos',
                'format' => 'raw',
                'value' => function ($model, $key, $index, $column) {
                    return Html::a("({$model->tela->getCantidadModelos()}) Modelos", ['/galeria/ver-disenios', 'tela_id' => $model->tela_id], ['class' => 'btn btn-default']);
                }
            ],
//            [
//                'label' => 'Lisos',
...
//            ],
            [
                ...
                'format' => 'raw',
                'value' => function ($model, $key, $index, $column) {
                    return Html::a('Exportar', ['/gallery-image/exportar', 'tela_id' => $model->tela_id], ['class' => 'btn btn-default']);
                }
            ],
//            [
////                'label' => 'Diseños',
...
//                    return Yii::$app->controller->renderPartial('/lisos/_disenios', ['model' => $lisoModel, 'categoria_padre' => $model->categoria->categoria_padre]);
                    return Yii::$app->controller->renderPartial('/tela/_lisoForm', ['model' => $model->tela]);
                },
                'headerOptions' => ['class' => 'kartik-sheet-style'],
                'expandOneOnly' => true,
